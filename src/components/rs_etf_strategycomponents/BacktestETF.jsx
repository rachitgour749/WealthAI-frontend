...
const Backtest = ({
  ...
  // ETF Selection State
  const [availableETFs, setAvailableETFs] = useState([]); // All ETFs for dropdown
  const [selectedETFs, setSelectedETFs] = useState([]); // Selected ETFs
  ...
  // Default parameter values (only visible parameters) - Adjusted for ETFs
  const [maxPositions, setMaxPositions] = useState(8); // Lower for ETFs
  ...
  const [stopLossPct, setStopLossPct] = useState(8.0); // Tighter for ETFs
  ...
  const [etfUniverse, setETFUniverse] = useState("ALL_ETFS"); // Default ETF universe
  ...
  const calculateDateRange = useCallback(async () => {
    ...
    try {
      ...
      const response = await rsETFStrategyService.calculateDateRange(
        selectedETFs.map((etf) => etf.value)
      );
      ...
      if (response && response.start_date && response.end_date) {
        setDateRange({
          ...
          years: response.years || 0,
        });
        // Set custom dates to match the calculated range initially
        setCustomStartDate(response.start_date);
        setCustomEndDate(response.end_date);
        console.log("Date range calculated:", response);
        console.log(
          "Set custom dates to:",
          response.start_date,
          "and",
  ...
  // Auto-load ETFs when component mounts so dropdown has options
  useEffect(() => {
    const loadDefaultETFs = async () => {
      try {
        setLoadingETFs(true);
        const response = await rsETFStrategyService.getAvailableETFs("ALL_ETFS");

        if (response.success && response.etfs) {
          const etfs = response.etfs.map((etf) => ({
            value: etf.symbol || etf.value,
            label: `${etf.symbol || etf.value}`,
          }));

          setAvailableETFs(etfs);
          setETFUniverse("ALL_ETFS");
        }
      } catch (error) {
        console.error("Error loading default ETFs:", error);
        message.error("Failed to load ETFs. Please check your connection.");
      } finally {
        setLoadingETFs(false);
      }
    };

    // Load default ETFs on component mount
    loadDefaultETFs();
  }, []); // Empty array = runs once on mount

  // Calculate date range when ETFs change
  useEffect(() => {
  ...
  useEffect(() => {
    if (initialConfig) {
      ...
      if (initialConfig.etf_universe !== undefined)
        setETFUniverse(initialConfig.etf_universe);

      // Restore selected ETFs
      if (
        initialConfig.custom_etfs &&
        Array.isArray(initialConfig.custom_etfs)
      ) {
        setSelectedETFs(initialConfig.custom_etfs);
      }

      // ETFs don't use universe checkboxes like stocks
  ...
  // ETFs don't use universe selection like stocks - just load all available ETFs
  ...
  const handleRunBacktest = async (values) => {
    ...
    try {
      ...
      if (!startDate || !endDate) {
        setError("Please select ETFs first or enable custom dates");
        setIsRunning(false);
        return;
      }
      ...
      const backtestParams = {
        ...
        etf_universe: etfUniverse, // Default: 'ALL_ETFS'
        custom_etfs:
          selectedETFs.length > 0 ? selectedETFs.map((s) => s.value) : null, // Custom ETF selection
        ...
        user_id: window.user?.email || "default_user", // Get user from window or context
        // Removed: max_holding_period, min_price, min_turnover (use backend defaults)
      };

      const result = await rsETFStrategyService.runBacktest(backtestParams);
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {onSaveStrategy && (
              <button
                ...
                disabled={activeSetupStep < 2 || selectedETFs.length === 0}
          ...
          <Select
            ...
            placeholder={
              loadingETFs
                ...
                : "Loading available ETFs..."
            }
            isLoading={loadingETFs}
            isDisabled={loadingETFs}
            className="mb-4"
            noOptionsMessage={() =>
              loadingETFs
                ? "Loading..."
                : "No ETFs available. Please check your connection."
            }
            styles={{
              control: (base) => ({
                ...base,
          ...
          {availableETFs.length > 0 && (
            <p className="text-xs text-gray-500 mt-2">
              {selectedETFs.length > 0
                ? `${selectedETFs.length} ETF(s) selected. You can add/remove ETFs manually.`
                : `Available: ${availableETFs.length} ETFs. Select ETFs from the dropdown.`}
            </p>
          )}
        </div>
      </div>
