...
const BestEtfCombinationModal = ({ isOpen, onClose, onSelectCombination }) => {
  ...
  const bestCombinations = [
    {
      id: 1,
      etfCombination: ['BANKBEES', 'CPSEETF', 'MON100', 'JUNIORBEES'],
      cagr: 19.10,
      brokerage: 0.1,
      timeFrame: '22-07-2016 - 22-07-2025',
      duration: '9 years'
    },
  ...
  const handleApplyCombination = () => {
    if (selectedCombination) {
      // Convert ETF names to the format expected by the Select component
      const etfOptions = selectedCombination.etfCombination.map(etf => ({
        value: etf,
        label: etf
      }));
            ...
            {bestCombinations.map((combination) => (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {/* ETF Combination */}
                  <div>
                    <h4 className="text-sm font-semibold text-gray-600 mb-2">ETF Combination</h4>
                    <div className="flex flex-wrap gap-1">
                      {combination.etfCombination.map((etf, index) => (
                        <span
                          key={index}
...
export default BestEtfCombinationModal;
