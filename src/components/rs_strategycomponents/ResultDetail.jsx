...
const ResultDetail = ({ backtestResult, onBackToSetup, onBack, onNewBacktest }) => {
  ...
  const saveStrategyParameters = async (customName) => {
    try {
      ...
      const strategyParams = {
        ...
        user_id: user?.email || '',
        config_id: backtestResult?.config_id || null,
        backtest_id: backtestResult?.backtest_id || backtestResult?.id || null,
        start_date: startDate,
        end_date: endDate,
        stock_universe: configData.stock_universe || "NIFTY500",
        backtest_results: {
      ...
      if (response.data.success) {
        ...
        const strategiesResponse = await axios.get(`/api/get-saved-rs-strategies/${encodeURIComponent(email)}`);
