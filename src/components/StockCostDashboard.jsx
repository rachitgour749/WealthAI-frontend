...
const StockCostsDashboard = () => {
  ...
  const loadCostsData = async () => {
    try {
      ...
      // Load costs summary
      const summaryResponse = await axios.get(buildApiUrl('STOCKS_COSTS_SUMMARY'));
      ...
      const analysisResponse = await axios.get(buildApiUrl('STOCKS_COSTS_ANALYSIS'));
      ...
      const breakdownResponse = await axios.get(buildApiUrl('STOCKS_COSTS_BREAKDOWN'));
