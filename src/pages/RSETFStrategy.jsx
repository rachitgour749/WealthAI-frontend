...
import BacktestETF from '../components/rs_etf_strategycomponents/BacktestETF';                 
import ResultsETF from '../components/rs_etf_strategycomponents/ResultsETF';
import ResultDetailETF from '../components/rs_etf_strategycomponents/ResultDetailETF';
...
function RSETFStrategyContent({ onBack, strategyType = "RS ETF Strategy", onSubPageChange }) {
  ...
  const { user } = useAuth();
  // Using rsETFStrategyService instead of context
  ...
  const loadBacktests = async () => {
    try {
      setLoading(true);
      const response = await rsETFStrategyService.getBacktestResults(user?.email);
  ...
  const fetchSavedStrategies = async () => {
    try {
      ...
      const apiUrl = `${API_BASE_URL}/api/get-saved-rs-etf-strategies/${user.email}`; // Update API endpoint for ETF strategies
  ...
  const handleStopStrategy = async (strategy) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/stop-rs-etf-strategy`, {
  ...
  const handleRestartStrategy = async (strategy) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/restart-rs-etf-strategy`, {
        strategy_id: strategy.id,
        user_id: user.email
      });
  ...
  const handleDeleteStrategy = async (strategy) => {
    try {
      const response = await axios.delete(`${API_BASE_URL}/api/delete-rs-etf-strategy/${strategy.id}`);
  ...
  const handleClientRemove = async (clientId) => {
    try {
      ...
      // Update RS ETF strategy client information
      const response = await axios.post(`${API_BASE_URL}/api/update-rs-client-information`, requestBody);
...
const hasSelectedClients = selectedIds.length > 0;
  ...
  const handleSaveStrategyFromUniverse = (config) => {
    if (!config || !config.selectedETFs || config.selectedETFs.length === 0) {
    ...
    setCustomStrategyName(`RS ETF Strategy - ${config.selectedETFs.length} ETFs - ${timestamp}`);
    setIsSaveStrategyPopupOpen(true);
    setSaveFromUniverseSelection(true);
  };

  // Save RS ETF strategy parameters
  const saveRsETFStrategyParameters = async (customName) => {
    ...
    try {
      ...
      const strategyParams = {
        ...
        end_date: endDate,
        etf_universe: currentBacktestConfig.etfUniverse || "ALL_ETFS",
        backtest_results: {
          total_return: 0,
          cagr: 0,
          sharpe_ratio: 0,
        ...
        strategy_config: {
          ...
          max_positions: currentBacktestConfig.maxPositions || 8,
          position_size_pct: currentBacktestConfig.positionSizePct || 5.0,
          stop_loss_pct: currentBacktestConfig.stopLossPct || 8.0,
          total_capital: currentBacktestConfig.totalCapital || 1000000.0,
          stock_universe: currentBacktestConfig.etfUniverse || "ALL_ETFS",
        ...
        tickers: currentBacktestConfig.selectedETFs.map(etf => etf.value),
      ...
      if (response.data.success) {
        message.success('RS ETF Strategy saved successfully!');
        setSavedStrategyName(customName);
        
        // Fetch the saved strategy to get its ID and details
        const strategiesResponse = await axios.get(`${API_BASE_URL}/api/get-saved-rs-etf-strategies/${user.email}`);
                ...
                <table className="min-w-full bg-white rounded-lg border border-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ETF Universe</th>
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Choose a descriptive name for your RS ETF strategy.
                </p>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  onClick={async () => {
                    ...
                    await saveRsETFStrategyParameters(customStrategyName.trim());
                  }}
        ...
        <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-teal-500 to-emerald-500">
              <h2 className="text-xl font-bold text-white">RS ETF Strategy</h2>
              <button
            ...
            <div className="flex-1 overflow-y-auto">
              <iframe
                ...
                title="RS ETF Strategy Information"
              />
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[10002] p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              {/* Content */}
              <div className="mb-6">
                <p className="text-sm text-gray-600 mb-2">Your strategy has been saved:</p>
                ...
                <button
                  onClick={() => {
                    if (recentlySavedStrategy) {
                      setWebHookStrategyType('RS ETF Strategy');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[350px] max-h-[90vh] overflow-hidden px-[10px] py-[10px]">
            <WebHook
              ...
              strategyType={webHookStrategyType || "RS ETF Strategy"}
              userEmail={user?.email || 'test@test.com'}
              selectedEtfs={selectedStrategy ? selectedStrategy.tickers : []}
              selectedStrategy={selectedStrategy}
              strategyParams={selectedStrategy ? {
              ...
              onDeploymentSuccess={() => {
                fetchSavedStrategies();
                message.success('RS ETF Strategy deployed successfully!');
              }}
            />
          </div>
        </div>
...
export default function RSETFStrategy({ onBack, onSubPageChange }) {
  return (
    <RSETFStrategyContent onBack={onBack} strategyType="RS ETF Strategy" onSubPageChange={onSubPageChange} />
  );
}
